...
export default function LogScreen() {
    ...
    useEffect(() => {
        ...
        if (sessionId) {
            ...
            // Function to update timer based on the original start time
            const tick = () => {
            ...
            // Set interval and keep reference
    ...
    }, [activeSession?.id, activeSession?.startTime]); // Only reset if the session ID or start time actually changes
    ...
    const checkIfBodyweight = (exerciseOrObj) => {
        const exerciseName = typeof exerciseOrObj === 'string' ? exerciseOrObj : (exerciseOrObj?.name || '');
    ...
    const getExerciseHistory = (exerciseOrObj) => {
        const exerciseName = typeof exerciseOrObj === 'string' ? exerciseOrObj : (exerciseOrObj?.name || '');
        const dbEntry = EXERCISE_DATABASE.find(e => e.name === exerciseName);
        const isTimed = dbEntry?.isTimed;
        
        const history = workouts
            .filter(w => (typeof w.exercise === 'string' ? w.exercise : w.exercise?.name) === exerciseName)
        ...
        try {
            ...
            session.exercises.forEach(ex => {
                ...
                const totalVolume = completedSets.reduce((sum, s) => {
                    if (isTimed) {
                        return sum + (parseInt(s.duration) || 0) * 1; // 1kg equivalent volume per second
                    }
                ...
                newIndividualWorkouts.push({
                    ...
                    isPR: false,
                    isBodyweight: isBW,
                    isTimed: isTimed // Store flag for analytics
                });
            });
            ...
            const muscles = [...new Set(newIndividualWorkouts.flatMap(w => [w.mainMuscle, w.subMuscle]))];
    ...
    const filteredExercises = useMemo(() => {
        ...
        const getName = (ex) => typeof ex === 'string' ? ex : (ex?.name || 'Unknown');

        if (!searchQuery) {
            return [...all].sort((a, b) => getName(a).localeCompare(getName(b)));
        }
        
        const q = searchQuery.toLowerCase();
        return all
            .filter(ex => getName(ex).toLowerCase().includes(q))
        ...
        <View style={[styles.container, { backgroundColor: colors.background }]}>
            <View style={[styles.header, { 
                ...
                {activeSession ? (
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', width: '100%', paddingHorizontal: 20, paddingBottom: 10 }}>
                        <View style={{ width: 40 }} />
...
const styles = StyleSheet.create({
    ...
    summaryMusclesRow: { marginTop: 20, paddingHorizontal: 10 },
    summaryMusclesLabel: { fontSize: 10, fontWeight: 'bold', letterSpacing: 1, marginBottom: 8 },
    summaryMusclesTags: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 },
    summaryMuscleTag: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, borderWidth: 1 },
    summaryMuscleTagText: { fontSize: 9, fontWeight: 'bold' },
    ...
    historyItem: { padding: 16, borderRadius: 12, borderWidth: 1, marginBottom: 12 },
